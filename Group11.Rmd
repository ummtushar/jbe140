...
```{r}
# Load the datasets
data_2007 <- read.csv("data/us_alliances_filtered_no_bankruptcies.csv")
data_2009 <- read.csv("data/us_alliances_filtered2009_no_bankruptcies.csv")
...
# One Mode Network Graph
```{r}
data_2007 <- read.csv("data/us_alliances_filtered_no_bankruptcies.csv")
data_2009 <- read.csv("data/us_alliances_filtered2009_no_bankruptcies.csv")

## prepare the network data for 1983-2007
us_alliances_net_1983_2007 <- data_2007 %>%
  select(participants, deal_number) %>%
  as.matrix()

us_alliances_graph_1983_2007 <- graph_from_data_frame(us_alliances_net_1983_2007, directed = FALSE)

# Convert to data frame if necessary
us_alliances_net_1983_2007 <- as.data.frame(us_alliances_net_1983_2007)

# Then proceed with counting unique participants
unique_participants <- unique(us_alliances_net_1983_2007$participants)
num_unique_participants <- length(unique_participants)

# Print the number of unique participants
print(num_unique_participants)

plot(us_alliances_graph_1983_2007, 
     vertex.label = NA, 
     vertex.size = 3, 
     main= "Interorganisational Relationship 1983-2007 (Pre Crisis)")

# Prepare the network data for 1983-2009
us_alliances_net_1983_2009 <- data_2009 %>%
  select(participants, deal_number) %>%
  as.matrix()

# Create the graph for 1983-2009
us_alliances_graph_1983_2009 <- graph_from_data_frame(us_alliances_net_1983_2009, directed = FALSE)

# Convert to data frame if necessary
us_alliances_net_1983_2009 <- as.data.frame(us_alliances_net_1983_2009)

# Count unique participants for the post-crisis period
unique_participants_2009 <- unique(us_alliances_net_1983_2009$participants)
num_unique_participants_2009 <- length(unique_participants_2009)

# Print the number of unique participants for the post-crisis period
print(num_unique_participants_2009)

# Set the random seed for reproducibility

set.seed(689)
# Plot the graph for the post-crisis period
plot(us_alliances_graph_1983_2009, 
     vertex.label = NA, 
     vertex.size = 3, 
     main = "Interorganisational Relationship 1983 - 2009 (Post Crisis)")
